...
case "$ROOT" in
    /mnt/c/*|/mnt/d/*|/mnt/e/*)
        ...
        ;;
    *)
        log "OK filesystem: repository berada di filesystem Linux WSL"
        ;;
esac
...
if [ -f build/proof/freestanding_probe.o ]; then
    readelf -hW build/proof/freestanding_probe.o \
        > build/meta/m2-check-m1-object-readelf.txt

    grep -q 'Class:.*ELF64' \
        build/meta/m2-check-m1-object-readelf.txt \
        || fail "Object M1 bukan ELF64"

    grep -q 'Machine:.*Advanced Micro Devices X86-64' \
        build/meta/m2-check-m1-object-readelf.txt \
        || fail "Object M1 bukan x86_64"
...
if find /usr/share -type f \
    \( -name 'OVMF_CODE*.fd' -o -name 'OVMF_VARS*.fd' \) \
    2>/dev/null | grep -q OVMF; then
